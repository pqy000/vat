...
import numpy as np
from model.TCNmodel import TCN, MTNet, MTForNet
...
def trainMTL(x_train, y_train, x_val, y_val, x_test, y_test, opt):
    ...
    mtnet = MTForNet(in_channel=x_train.shape[2], nb_classes=opt.nb_class,
                     horizon = int(x_train.shape[1]*opt.horizon)).cuda()
    ...
    for epoch in range(tot_epochs):
        ...
        for i, (data_labeled, data_unlabel) in enumerate(zip(cycle(train_loader_label), train_loader)):
            ...
            prediction = y_hat_classification.argmax(-1)
            ...
            loss_unlabel.append(loss_forecasting.item())
            acc_unlabel.append(0.0)
