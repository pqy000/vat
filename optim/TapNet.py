...
import numpy as np
from model.TCNmodel import TCN, MTNet, TapNet
from itertools import cycle
import torch.nn as nn

def trainTapNet(x_train, y_train, x_val, y_val, x_test, y_test, opt):
    K = opt.K
    ...
    mtnet = TapNet(nfeat=x_train.shape[2],
                   ...
                   kernels=opt.kernels,
                   use_metric=opt.use_metric,
                   use_rp=opt.use_rp,
                   rp_params=opt.rp_params,
                   lstm_dim=opt.lstm_dim).cuda()
